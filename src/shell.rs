...
impl Shell {
    pub fn new() -> Self { Self { prev_dir: None, history: Vec::new() } }

    pub fn push_history(&mut self, line: &str) { self.history.push(line.to_string()); }
    ...
    fn exec_one(&mut self, raw: &str) -> Result<(), String> {
        // Simple redirection parsing: support one '<' and/or one '>'
        let mut cmd_part = raw.to_string();
        let mut infile: Option<String> = None;
        let mut outfile: Option<String> = None;

        // Parse output redirection first (rightmost)
        if let Some(pos) = cmd_part.rfind('>') {
            let target = cmd_part[pos + 1..].trim().to_string();
            if target.is_empty() { return Err("syntax error: expected filename after '>'".into()); }
            outfile = Some(target);
        ...
        // Parse input redirection (rightmost of remaining)
        if let Some(pos) = cmd_part.rfind('<') {
            let target = cmd_part[pos + 1..].trim().to_string();
            if target.is_empty() { return Err("syntax error: expected filename after '<'".into()); }
            infile = Some(target);
        ...
        for (i, stage) in stages.iter().enumerate() {
            // argv parse + env/tilde expansion
            ...
            // Next stage reads from previous output
            current_in = Box::new(io::Cursor::new(last_output.clone()));
            if i + 1 == stages.len() { break; }
        }
        ...
        if let Some(of) = outfile {
            ...
            io::stdout().write_all(&last_output).map_err(|e| e.to_string())?;
            if !last_output.ends_with(b"\n") { println!(); }
        }

        Ok(())
    }

    fn dispatch(&mut self, argv: &[String], input: &mut dyn Read, output: &mut dyn Write) -> Result<(), String> {
        match argv[0].as_str() {
            "exit"    => std::process::exit(0),
            "echo"    => builtins::echo::run(&argv[1..]),
    ...
    pub fn chdir(&mut self, to: PathBuf) -> Result<(), String> {
        ...
        std::env::set_current_dir(&to).map_err(|e| e.to_string())?;
        self.prev_dir = Some(cur);
        Ok(())
    }

    pub fn prev_dir(&self) -> Option<PathBuf> { self.prev_dir.clone() }
}
